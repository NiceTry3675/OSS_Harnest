...
## 3차 — 무문서 프로브 기반 drop (2026-08-22, PROTOCOL §5 재산정 1회)

1차 프로브(41케이스) 결과 무문서 정답률 **48.8% = "주의"**. §5 절차대로 **무문서 만점 15건**을 drop — 만점 사유가 오염(학습 데이터)이든 일반지식 답변 가능성이든, responder가 문서 없이 정답에 도달하는 케이스는 문서의 기여를 측정할 수 없다. 개별 목록·채점 근거는 `probe_drops.json`.

drop: #1947, #8445, #9129, #10387, #15694, #17690, #19752, #31559, #32757, #33811, #34662, #38360, #43880, #53359, #53760

재구성: **26케이스 = 가시 17 + 홀드아웃 9** (분할 규칙 동일 적용, 홀드아웃이 목표 하한 10을 1건 하회 — 기록). 재산정 프로브는 캐시 재사용으로 산출.
