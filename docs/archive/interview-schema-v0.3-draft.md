...
> [!WARNING]
> 이 문서는 구현되지 않은 과거 설계 초안이며 현재 계약이 아니다.
> 현행 입력 계약은 [`packages/contracts/src/interview.ts`](../../packages/contracts/src/interview.ts)와 관련 테스트를 정본으로 삼는다.

> 아래 내용은 프론트와 생성 엔진 사이의 입력 계약으로 제안했던 당시 원문이다.
> 현재 구현이나 로드맵으로 해석하지 않으며, 내부 링크와 절 번호도 역사적 상태로 보존한다.
...
8. v0.4 백로그: 빈 `cases`/전량 홀드아웃 금지 규칙과 최소·최대 케이스 수, `judge.model` 검증 규칙(2026-08-22 모델 정책 반영: 체험 = `gemini-3.7-flash` 고정, BYO = 기본 프리셋 4종 `claude-fable-5`·`claude-opus-5`·`gpt-5.6-sol`·`gemini-3.1-pro` + 커스텀 프로바이더 자유 입력 — 허용 목록 검증이 아니라 **형식 검증만**, 품질 심사는 시험관 검증 배터리·캘리브레이션 소관. 저지 모델은 동결 대상이므로 승인 전 확정 — SPEC §12 미결 2·7), **panel 패널원 배열**(이질 패널 — 패널원별 모델·프롬프트 변형, PHILOSOPHY §2 "관점이 다른 judge 패널"의 온전한 표현), case_answering 세분화(케이스별 가중치, 부분 점수 척도 확장, grader 패널 적용 여부 재검토), semver 호환성 정책·템플릿 버전 업그레이드 경로(예시-schemaVersion 동기화 규칙 포함), `origin: "generated"`에 `role: "material"` 답변 1개 이상 요구 규칙
