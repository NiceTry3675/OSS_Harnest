...
`fly.toml`의 `http_service.http_options.idle_timeout`(900초, Fly가 허용하는 최댓값)은 공유 키
프록시 때문에 필요합니다. 프록시는 벤더 응답을 다 받은 뒤에야 첫 바이트를 보내므로, Fly edge의 기본
유휴 한도(60초)가 서버의 상류 읽기 한도보다 짧으면 edge가 먼저 연결을 끊습니다. 벤더 쪽 생성·과금은
완주하는데 사용자는 산출물을 잃고, 브라우저는 그 실패를 서버의 504와 구분할 수 없습니다. 900초를
넘기면 배포 자체가 거부되므로, 서버의 읽기 한도는 출력 토큰 상한에 비례하되
`HARNEST_PROXY_MAX_TIMEOUT`(기본 780초)에서 멈추고 브라우저는 거기에 60초를 더한 840초까지
기다립니다. 상한을 올리려면 edge 한도가 브라우저 대기 시간(상한 + 60초)보다 커야 하므로 Fly에서는
840초가 한계이고, 다른 호스팅에서는 같은 이유로 edge·리버스 프록시의 유휴 한도를 함께 맞춰야 합니다.
...
| `HARNEST_PROXY_TIMEOUT` | `300` | 프록시가 벤더 응답을 기다리는 초의 바닥. 실제 한도는 요청의 출력 토큰 상한 × 30ms가 더 크면 그만큼(브라우저 클라이언트와 같은 산식 — 20,000토큰이면 600초). 넘기면 504 |
| `HARNEST_PROXY_MAX_TIMEOUT` | `780` | 위 한도의 상한(초). 배포 edge의 유휴 한도(`fly.toml` `idle_timeout` 900초, Fly 최댓값)보다 브라우저 여유 60초 이상 작아야 한다. 올리면 브라우저 클라이언트의 상한(`llm.ts`)도 함께 올려야 한다 |
...
- 벤더 응답이 읽기 한도 안에 오지 않으면 504를 돌려주고 서버 로그에 남깁니다. 읽기 한도는
  ...
  끊지 않습니다. 다만 배포 edge의 유휴 한도 아래에 머물도록 `HARNEST_PROXY_MAX_TIMEOUT`(기본
  780초, 26,000토큰 분량)에서 멈추고, 브라우저 클라이언트도 같은 상한을 알고 60초 여유를 더해
  기다립니다. 그보다 긴 생성은 본인 키의 직행 경로로 실행해야 합니다. 끊긴 생성도 벤더 쪽에서는 완주·과금되므로 클라이언트는 504를 재시도하지 않습니다
